...
describe('Messages', () => {
  ...
  it('deletes a message and checks that the response message is correct', async () => {
    ...
    await waitFor(() => {
        const deleteBtn = screen.getByTestId(6 + "delete-message-modal");
        expect(deleteBtn).toBeInTheDocument();
        fireEvent.click(deleteBtn);
    })
    
    // await waitFor(() => {
    //     const confirmDeleteBtn = screen.getByTestId(6 + "confirm-delete-message");
    //     fireEvent.click(confirmDeleteBtn);
    // })

    // const toastMessage = (await screen.findByRole("alert")).lastChild;

    // expect(toastMessage.textContent).toBe("Message deleted successfully"); // user successfully deletes message (T38.1)
