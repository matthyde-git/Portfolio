...
describe('Messages', () => {
  ...
  it('renders the message board for a project, updates a message and checks the response message', async () => {
    ...
    const updatedMessage = screen.getByTestId(8 + "update-message");
    expect(updatedMessage).toBeRequired();  // prevents empty messages (T37.2)

    updatedMessage.textContent = "New message";

    const confirmUpdateBtn = screen.getByTestId(8 + "update-message-btn");
    ...
    expect(toastMessage.textContent).toBe("Message updated successfully");  // user successfully updates message (T37.1)
