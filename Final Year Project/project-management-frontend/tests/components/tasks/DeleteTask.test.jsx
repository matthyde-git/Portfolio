...
describe('Tasks', () => {
  ...
  it('deletes a task and checks that the response message is correct', async () => {
    ...
    // await waitFor(() => {
    //     const deleteTaskBtn = screen.getByTestId(8 + "delete-task-btn");
    //     fireEvent.click(deleteTaskBtn);
    // })

    // await waitFor(() => {
    //     const confirmDeleteBtn = screen.getByTestId(8 + "confirm-delete-task");
    //     fireEvent.click(confirmDeleteBtn);
    // })

    // const toastMessage = (await screen.findByRole("alert")).lastChild;

    // expect(toastMessage.textContent).toBe("Task deleted successfully");   // user successfully deletes task (T23.1)
