...
describe('Tasks', () => {
  ...
  it('renders the tasks for a project where the user is the team leader, updates a task with invalid information and checks the response message', async () => {
    ...
    await waitFor(() => {
        const showFormBtn = screen.getByTestId(9 + "update-task-btn");
        fireEvent.click(showFormBtn);
    })

    const status = screen.getByTestId(9 + "update-task-status");
    const deadline = screen.getByTestId(9 + "update-task-deadline");
    const updateBtn = screen.getByTestId(9 + "update-task-form-btn");
    ...
    expect(toastMessage.textContent).toBe("Deadline cannot be in the past");  // user fails to update task (T22.2)
